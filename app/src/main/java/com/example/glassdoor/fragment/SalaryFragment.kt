...
import com.example.glassdoor.POJO.*
import com.example.glassdoor.R
import com.example.glassdoor.adapter.SalaryListAdapter
...
class SalaryFragment : Fragment() {
    ...
    internal var moAdapter: SalaryListAdapter? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val iView = inflater!!.inflate(R.layout.fragment_salary, container, false)
        recyclerView = iView.findViewById<RecyclerView>(R.id.rvSalaryList)
        getSalaryData()

        return iView
    }

    private fun getSalaryData(): ArrayList<Salary> {
        var sList = ArrayList<Salary>()
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (myresults != null) {
                            for (i in 0 until myresults.size) {
                                ...
                                data = resultList.salary
                                Log.d(TAG, "Success:" + data)
                                if (data != null) {
                                    sList.add(Salary(data.basePay, data.employerName, data.employmentStatus, data.jobTitle,
                                            data.location, data.meanBasePay, data.payPeriod))

                                    moAdapter = SalaryListAdapter(sList)
